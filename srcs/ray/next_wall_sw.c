...
/*   Updated: 2023/08/13 20:50:00 by mjulliat         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_wall	ft_next_wall_sw(float rad, t_real pos, char **map)
{
	...
	if (w.hypo.y == w.hypo.x)
	{
		...
		{
			if (map[(int)pos.y + 1][(int)(pos.x - 1 - w.of7.x)] == 'D')
			{
				wall.door = true;
				return (wall);
			}
	...
	else if (w.hypo.y < w.hypo.x)
	{
		...
		if (map[(int)pos.y + 1] != NULL)
		{
			if (map[(int)pos.y + 1][(int)(pos.x - w.of7.x)] == 'D')
			{
				wall.door = true;
				return (wall);
			}
	...
	else
	{
		...
		{
			if (map[(int)pos.y][(int)(pos.x - 1 - w.of7.x)] == 'D')
			{
				wall.door = true;
				return (wall);
			}
